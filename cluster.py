# This script contains code to load, preprocess, and cluster polygons by distance
# and/or spatial relationships to land parcels.
...
def load_parcels(parcel_path, piecewise=False, feather=False, **kwargs):
    ...
    """
    if feather:
        # Suppress CRS warning:
        warnings.filterwarnings('ignore', message=str(parcel_path) + ".crs")
        try:
            parcels = gpd.read_feather(parcel_path).set_crs(3071)
        except ValueError as exc:
            if "Missing geo metadata" not in str(exc):
                raise

            # Fallback for legacy files that were saved without geo metadata.
            df = pd.read_feather(parcel_path)

            def _ensure_shapely_geometry(geom):
                if isinstance(geom, BaseGeometry):
                    return geom
                if geom is None or (isinstance(geom, float) and np.isnan(geom)):
                    return None
                if isinstance(geom, (bytes, bytearray)):
                    return shapely.from_wkb(geom)
                if isinstance(geom, str):
                    stripped = geom.strip()
                    if stripped[:2].lower() == "b'" or stripped[:2].lower() == 'b"':
                        try:
                            geom_bytes = literal_eval(stripped)
                            return shapely.from_wkb(geom_bytes)
                        except (SyntaxError, ValueError, TypeError):
                            pass
                    return shapely.from_wkt(geom)
                return shapely.from_wkt(str(geom))

            df["geometry"] = df["geometry"].apply(_ensure_shapely_geometry)
            parcels = gpd.GeoDataFrame(df, geometry="geometry", crs=3071)
    elif piecewise:
...
def load_clusters(cluster_path):
    ...
    Return: loaded cluster as a geo DataFrame, with spatial indices
    """
    df = pd.read_csv(
        cluster_path,
...
def cluster(
    ...
    cluster_subgraphs = [
        polygon_graph.subgraph(c).copy()
        for c in sorted(nx.connected_components(polygon_graph), key=lambda c: sorted(c))
    ]
    ...
    clusters["parcel_indices"] = (
        ...
        .apply(lambda x: sorted(set([item for sublist in x for item in sublist])))
    )
    ...
    clusters["parcel_geoms"] = (clusters["parcel_geoms"]
                                ...
                                .apply(lambda x: sorted(set([multipolygon for multipolygon_list in x for multipolygon in multipolygon_list])))
    ...
    clusters["jpeg_names"] = clusters["jpeg_names"].apply(
        lambda x: sorted(set([jpeg_name for jpeg_name_list in x for jpeg_name in jpeg_name_list])))
...
if __name__ == "__main__":
    ...
    outside_ewg_annotations = load_polygons(data_path / 'Annotations/full_state_cf_annotations.geojson',
                                            counties, image_bound_map)
    inside_ewg_annotations = load_polygons(data_path / 'Annotations/ewg_region_train_labels.geojson',
                                            counties, image_bound_map)
    ...
    # 4. Run clustering for model predictions + different vectorization thresholds - this takes forever to run, need to fix
    # vec_threshes = np.array([0.25, 0.4, 0.5, 0.6, 0.75])


    # # Also run and save clusters for different models and vectorization thresholds
    # for model in tqdm(['three_band', 'four_band'], desc="Models"):
    #     for vec_thresh in tqdm(vec_threshes, desc="Vectorization thresholds"):
    #         print(f'Running clustering for {model} model with vector threshold {vec_thresh}')
    #         predictions = load_polygons(data_path / f"Model_predictions/{model}/{model}_full_state/{model}_all_predictions-{vec_thresh}.geojson",
    #                                     counties, image_bound_map)

    #         # Run clustering
    #         clusters = cluster(parcels, predictions)
    #         save_clusters(clusters, f"{model}_clusters_vec_thresh_{vec_thresh}.csv", cluster_path)
